function [H_MMSE_VP, Equalized_OFDM_Symbols] = MMSE_Virtual_Pilots(he_LS_Preamble ,y_r, Kset,mod, ppositions,dpositions, noise_power_OFDM_Symbols)
...
for i = 1:OFDM_Symbols
    if(i == 1)
        ...
        if isscalar(noise_power_OFDM_Symbols)
            sigma2 = noise_power_OFDM_Symbols;
        else
            sigma2 = noise_power_OFDM_Symbols(1,i);
        end

    H_MMSE_VP(:,i) = (R_HP / (R_PP + sigma2 * eye(subcarriers))) * P(:,i);
